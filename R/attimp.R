...
#'
#' @param vector of identities of objects in the group
#' 
...
#' 
#' @param lX vector of attribute flags (see the help file of \code{\link[COSA]{cosa}}).
#'`
...
#'
#' @param xmiss missing value flag (see the help file of \code{\link[COSA]{cosa}}).
#'
...
#    	rewrite X to matrix here!
   ...
   if (missing(lX) && !is.null(attr(X,'lX'))) { lXt=attr(X,'lX')} else {lXt=lX} 
   ...
   eps=1/maximp; disp=rep(0,ncol(X)); wtave=rep(0,ncol(X))                     
   for (k in 1:ncol(X)) { if (lXt[k]<=0) { wtave[k]=0; next}                   
      z=X[X[,k]<xmisst,k]; grp=group[X[group,k]<xmisst]                        
      if (length(grp)<3) { wtave[k]=0; next}                                   
      zg=X[grp,k]                                                              
      if (lXt[k] < 4) {                                                        
         disp[k]=IQR(z); if (disp[k]<=0) { wtave[k]=0; next}                   
         if (fast) { wtave[k]=1/(IQR(zg)/disp[k]+eps)}                         
         else { wti=rep(0,length(grp))                                         
            for (i in 1:length(grp)) { wti[i]=median(abs(zg[i]-zg))}           
            wtave[k]=1/(1.35*mean(wti)/disp[k]+eps)                            
         }
      }                                                                     
      else { u=unique(z); s=0; sq=0
         for (v in u) { t=length(z[z==v]); s=s+t; sq=sq+t**2}
         if (s <= 0) { disp[k]=0; wtave[k]=0; next}                  
         disp[k]=1-sq/s**2                            
         u=unique(zg); s=0; sq=0
         for (v in u) { t=length(zg[zg==v]); s=s+t; sq=sq+t**2}
         if (s <= 0) { wtave[k]=0;}                
         else { wtave[k]=1/((1-sq/s**2)/disp[k]+eps)}
      }                          
   }                                                                        
   o=order(-wtave); out=list(imp=wtave[o],att=o)
   if (times <= 0) {                               
      ...
      plot(range,wtave[o[range]],ylab='Importance',xlab='Ordered Attributes',ylim=ylim, 
         type='n')                                                             
      title(main)
      attimpline = wtave[o[range]]                                                              
      wttave=rep(0,length(wtave))                                              
      for (it in 1:times) { mr=trunc(nrow(X)*runif(length(group))+1)           
         for (k in 1:ncol(X)) { if (lXt[k]<=0) { wtave[k]=0; next}             
            if (disp[k]<=0) { wtave[k]=0; next}                                
