...
function(attimp,atts=attimp$att[1:min(40,length(attimp$att))],         
   ...
   o=1:length(atts); if (imporder) o=order(-imps)                              
   if (plot) {                                                                 
      if (horiz) { xl='Importance'; yl=''}                                     
      else { xl=''; yl='Importance'}                                           
      if (donames) {                                                           
         bplot(imps[o],names=as.character(att.names[attnums[o]]),              
            horiz=horiz,xlab=xl,ylab=yl,zlim=ylim,trans=trans,ntk=ntick,       
               names.size=names.size)                                          
        title(main)                                                            
      } else {                                                                   
         bplot(imps[o],ylab=yl,xlab=xl,zlim=ylim,                              
            horiz=horiz,trans=trans,ntk=ntick)                                 
         title(main)                                                           
      }                                                                        
   }                                                                           
   if (plot) {invisible(imps)} else {imps}                                     
}
